...
function getDataList(){
    if(props.itemApi != null){
        data.loading = true;
        api[props.itemApi]({page: data.page,limit: data.limit}).then(res => {
            if(res.errno === 0){
...
function search(){
    data.loading = true;
    api[props.searchApi]({ key: data.searchValue }).then(res => {
        if(res.errno === 0){
            data.searchValue = ""
...
function Deleted(item){
    ElMessageBox.confirm('确定删除吗？', '提示', {
        ...
        api[props.deleteApi](item).then(res => {
            if(res.errno === 0){
...
function Deletes(items){
    ElMessageBox.confirm('确定删除吗？', '提示', {
        ...
        api[props.deletesApi](items).then(res => {
            if(res.errno === 0){
                ElMessage.success("操作成功");
                getDataList()
                emit("change","delete")
...
function UpdateInfo(){
    ElMessageBox.confirm('确定修改吗？', '提示', {
        ...
        api[props.updateApi](props.editInfo).then(res => {
            if(res.errno === 0){
                data.isEdit = false
                ElMessage.success("操作成功");
                getDataList()
...
function AddInfo(){
    ...
    api[props.addApi](props.editInfo).then(res => {
        if(res.errno === 0){
            data.isAdd = false
            ElMessage.success("操作成功");
            getDataList()
